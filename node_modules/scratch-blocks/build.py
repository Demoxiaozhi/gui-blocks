...
class Gen_compressed(threading.Thread):
  ...
  Runs in a separate thread.
  """
  def __init__(self, search_paths_vertical, search_paths_horizontal, compileLocal = True):
    threading.Thread.__init__(self)
    self.search_paths_vertical = search_paths_vertical
    self.search_paths_horizontal = search_paths_horizontal
    self.compileLocal = compileLocal
  ...
  def gen_core(self, vertical):
    ...
    if self.compileLocal:
      pass  # TODO: if need to support parameters for local compile, add options
    else:
  ...
  def gen_blocks(self, block_type):
    ...
    if self.compileLocal:
      ...
      # hack generator.js
      #filenames.insert(0, os.path.join("build_hack", "blocks.js"))
      filenames.append(os.path.join("build_hack", "blocks.js"))

      filenames.append(os.path.join("core", "colours.js"))
      filenames.append(os.path.join("core", "constants.js"))
      # Remove Blockly.Blocks to be compatible with Blockly.
      remove = ["var Blockly={Blocks:{}};"]
      self.do_compile_local(target_filename, filenames, remove)
    else:
      # Define the parameters for the POST request.
      params = [
          ("compilation_level", "SIMPLE_OPTIMIZATIONS"),
  ...
  def gen_generator(self, language):
    ...
    if self.compileLocal:
      ...
      # hack generator.js
      filenames.insert(0, os.path.join("build_hack", "generator.js"))
  ...
  def do_compile_local(self,target_filename,filenames,remove):
      cfilenames = " ".join(filenames)

      command = 'java -jar ../closure-compiler-jar/closure-compiler-v20170521.jar --compilation_level SIMPLE_OPTIMIZATIONS --js ' + cfilenames + ' --js_output_file ' + target_filename
      ...
      if sys.platform.startswith('darwin'):
          for item in remove:
              os.system("sed -i '' 's/"+item+"//' "+target_filename)

          # remove comments
          # http://www.cnblogs.com/myyan/p/4849792.html
          os.system("sed -i '' '/^[ \t]*\/\*/,/.*\*\//d' " + target_filename)
      else: # linux
          for item in remove:
              os.system("sed -i 's/"+item+"//' "+target_filename)
    ...
    for f in json_files:
      ...
      if os.path.isfile(f):
          if sys.platform.startswith('darwin'):
              ...
              os.system("sed -i '' '/^'\\\''use strict.*/d' " + f);
              os.system("sed -i '' '/^goog.*/d' " + f);
          else: # linux
            # remove un-needed lines!!!
            os.system("sed -i '/^'\\\''use strict.*/d' " + f);
            os.system("sed -i '/^goog.*/d' " + f);
