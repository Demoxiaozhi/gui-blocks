...
Blockly.Arduino.workspaceToCode = function(workspace) {
  ...
  for (var x = 0, block; block = blocks[x]; x++) {
    if (block.type === 'dev_arduinoMain' || block.type === 'cubic_arduinoMain') { // 只针对arduinoMain翻译代码！！！
...
Blockly.Arduino.blockToCode = function(block) {
  ...
  */
  ...
  var code;
  if (!func) {   // not-yet-implemented or not supported blocks
    if (block.outputShape_ === null) {
      //code = Blockly.Arduino.noGeneratorCodeLine();
      code = this.noGeneratorCodeLine();
    } else {
      //code = Blockly.Arduino.noGeneratorCodeInline();
      code = this.noGeneratorCodeInline();
    }
  } else {
